...
//
// CAD vector files: DXF is parsed directly; DWG (AutoCAD binary) is converted to
// DXF in-browser via a WebAssembly LibreDWG build, then run through the DXF path.
...
async function renderDxf(file: File): Promise<LoadedPlan> {
  return rasterizeDxfText(await file.text());
}

// DWG is AutoCAD's binary format. We convert it to DXF in-browser via a WebAssembly
// build of LibreDWG (no server, no cloud — works offline), then reuse the DXF path.
// The .wasm is served from /public/libredwg/ (see public/libredwg/libredwg-web.wasm).
async function renderDwg(file: File): Promise<LoadedPlan> {
  const { LibreDwg } = await import("@mlightcad/libredwg-web");
  const lib = await LibreDwg.create("/libredwg");
  const buf = await file.arrayBuffer();
  const dxfBytes = lib.dwg_write_dxf(buf);
  if (!dxfBytes || dxfBytes.length === 0) {
    throw new Error(
      "Couldn't read this DWG. It may be an unsupported AutoCAD version or corrupt — " +
        "try Save As → DXF in AutoCAD, or export a PDF, and upload that.",
    );
  }
  const text = new TextDecoder().decode(dxfBytes);
  return rasterizeDxfText(text);
}

async function rasterizeDxfText(text: string): Promise<LoadedPlan> {
  ...
  // Rasterise the DXF's SVG into a clean dark-on-white plan image. The raster's
  // longest side is always ~target px REGARDLESS of the drawing's units — a plan
  // drawn in metres has tiny bbox numbers (e.g. 7.2), one in millimetres has big
  // ones (7200); both must render at full resolution. (The old `min(3, …)` cap
  // rendered metre-unit DWG/DXF at ~22px — unreadable.)
  const target = 1600;
  const scale = target / maxU;
...
export function loadPlanFile(file: File): Promise<LoadedPlan> {
  if (isDwg(file)) return renderDwg(file);
